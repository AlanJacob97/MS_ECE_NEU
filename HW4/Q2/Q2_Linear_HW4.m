...
model2=fitcsvm(x',L','KernelFunction','linear','OptimizeHyperparameters','BoxConstraint');
mdl1=crossval(model2,'KFold',10);
min_loss1=kfoldLoss(mdl1);

model3=fitcsvm(x',L','KernelFunction','linear','BoxConstraint',model2.BoxConstraints(1));
...
for l = 1:2 
    ...
    figure(3), plot(x_test(1,indices),x_test(2,indices),'.','MarkerFaceColor',colorList(l)); axis equal, hold on,
    ylabel('Feature value x2');
    xlabel('Feature value x1');
...
model4=fitcsvm(x_test',L_test','KernelFunction','linear','BoxConstraint',model2.BoxConstraints(1));
...
for l = 1:2 
    ...
    figure(4), plot(x_test(1,indices),x_test(2,indices),'.','MarkerFaceColor',colorList(l)); axis equal, hold on,
    ylabel('Feature value x2');
    xlabel('Feature value x1');
